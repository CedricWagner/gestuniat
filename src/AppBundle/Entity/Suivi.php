...
class Suivi
{
    ...
    {
        if ($this->dossier) {
            return '<a href="/contact/'.$this->dossier->getContact()->getId().'/dossier/'.$this->dossier->getId().'">Dossier "'.$this->dossier->getContact()->getNom().' '.$this->dossier->getContact()->getPrenom().' > '.$this->dossier->getNom().'"</a>';
        }
        if ($this->section) {
